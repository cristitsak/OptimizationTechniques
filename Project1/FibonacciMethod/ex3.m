...
% Εύρος τιμών l
lambda_values = linspace(0.001, 0.011, 200);

% Διάγραμμα l - πλήθος υπολογισμου f
for func_idx = 1:3
    figure;
    ...
    for i = 1:length(lambda_values)
        l = lambda_values(i);
        fib_obj = fibonacci(funcs{func_idx}, a0, b0, l);
        result = fib_obj.Calc();
    ...
    title(sprintf('Fibonacci Section Method - %s', func_names{func_idx}));
    grid on;
...
% Διάγραμμα για τιμές α,β για διαφορετικές τιμές l και άρα k
for func_idx = 1:3
    figure; 
    hold on;
    ...
    for i = 1:length(l_values)
        l = l_values(i);
        fib_obj = golden(funcs{func_idx}, a0, b0, l);
        result = fib_obj.Calc();
